...
If you want to use your own custom redis you can save redis url in the environment variable "REDIS_URL" the default url is "redis://localhost:6379"
...
With the gem, you do not need to manually add exchange rates. Calling update_rates will download the rates from the European Central Bank. The API is the same as the money gem. Feel free to use Money objects with the bank.
  ...
  # this will download the rates from ECB and save it to reds
  eu_bank.save_rates

  # this will load the rates from redis cache
  eu_bank.load_rates
